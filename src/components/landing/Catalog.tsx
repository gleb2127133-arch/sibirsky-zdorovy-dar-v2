...
type CatalogProps = { onOrder: (productId: string, quantity: number) => void };
...
const ProductCard = ({ p, onOrder }: { p: Product; onOrder: CatalogProps["onOrder"] }) => {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-white transition-all duration-200 hover:-translate-y-1 hover:border-primary/30 hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <button
          type="button"
          onClick={() => onOrder(p.id, qty)}
          className="btn-primary mt-4 w-full"
        >
          Заказать {qty > 1 ? `${qty} шт.` : ""}
  ...
  <section id="catalog" className="bg-white py-20 lg:py-28">
    <div className="container-narrow">
      ...
      <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {products.map((p) => (
          <ProductCard key={p.id} p={p} onOrder={onOrder} />
        ))}
      </div>
    </div>
  </section>
