...
- **Google Search Console** — сайт добавлен и подтверждён
- **Sitemap** — `/sitemap.xml` создан на сервере, добавлен в Вебмастер и Search Console
...
- Индексация началась, первые результаты ожидаются через 1–3 недели

---

## Блог (SEO-контент)

- **Страница блога:** `https://activepluse.ru/blog` — список статей
- **Статья:** `https://activepluse.ru/blog/:slug` — отдельная страница
- **Генератор:** `generate-article.cjs` — запускать: `node generate-article.cjs` (нужны env vars)
- **Cron:** каждый день в 09:00 МСК (06:00 UTC) — автоматически генерирует статью
- **Лог:** `/var/log/blog-generator.log` на сервере
- **Хранилище статей:** `/var/www/aktivplus/blog/*.json` (на сервере, не в git)
- **Рубрики:** спорт / научные исследования / здоровье / новости / применение
- **SEO:** 1500–2500 слов, ключевые слова, уникальный title+description на каждую статью
- **После генерации:** уведомление в Telegram со ссылкой на статью
- **Tailwind Typography** (`@tailwindcss/typography`) — для красивого отображения HTML статей
- Кнопка "Выбрать объём" в конце статьи → ведёт на `/#catalog`
...
### Правила SEO по Google (starter guide)
- **Title:** уникальный на каждой странице, описывает содержание, содержит ключевое слово
- **Description:** 1–2 предложения, уникальные, отражают суть — влияют на CTR
- **Контент:** достоверный, актуальный, полезный — не копировать, не переспамить ключами
- **Структура:** разбивать на разделы с h2/h3, параграфы, списки
- **Изображения:** alt-текст краткий и информативный, качественные фото рядом с релевантным текстом
- **URL:** описательные (`/blog/dgk-dlya-sporta`), группировка по темам
- **Canonical:** указывать на каждой странице чтобы избежать дублей
- **Ссылки:** якорный текст описывает целевую страницу, `rel="nofollow"` на ненадёжные источники
- **Sitemap:** отправлен в Search Console и Вебмастер ✅
- **Что НЕ влияет:** meta keywords, объём текста, порядок заголовков, расширение домена
- **Сроки:** от нескольких недель до нескольких месяцев — нужно терпение и регулярный контент
