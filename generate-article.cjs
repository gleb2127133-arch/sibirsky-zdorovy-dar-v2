...
ПРАВИЛА НАПИСАНИЯ:
1. Используй реальные научные данные — ищи через web_search актуальные исследования
2. Пиши живым, экспертным языком — не как Википедия, а как умный врач-консультант
3. Структура: вступление → 3-5 разделов с h2 → практические советы → вывод с мягкой отсылкой к продукту
4. Длина: 1500–2500 слов
5. SEO: естественно вшивай ключевые слова: "дигидрокверцетин", "ДГК", "таксифолин", "антиоксидант из лиственницы", "купить дигидрокверцетин", "дигидрокверцетин польза", "природный антиоксидант"
6. Не пиши "лечит", "лекарство" — пиши "поддерживает", "способствует", "помогает организму"
7. Упоминай АктивПлюс 1-2 раза ненавязчиво, не в каждом абзаце
8. В конце добавь призыв посетить https://activepluse.ru

ПРАВИЛА SEO ПО GOOGLE (обязательно):
— ЗАГОЛОВОК (title): уникальный, 50-60 символов, главное ключевое слово ближе к началу, не дублирует заголовки h2/h3 внутри статьи
— META DESCRIPTION: 150-160 символов, содержит главный ключевой запрос, побуждает кликнуть, уникальна для каждой статьи
— ЗАГОЛОВКИ H2/H3: каждый раздел имеет свой заголовок, раскрывает тему раздела, ключевые слова вписаны органично
— УНИКАЛЬНОСТЬ: статья должна давать реальную ценность — факты, данные исследований, практические советы. Не переписывай другие статьи
— КЛЮЧЕВЫЕ СЛОВА: используй естественно, без переспама. Норма: 1-2% от объёма текста. Не вставляй в каждое предложение
— СТРУКТУРА: чёткие абзацы, списки ul/li где уместно, blockquote для важных фактов/цитат. Текст должно быть легко сканировать
— ССЫЛКИ: не добавляй внешние ссылки в content, только внутренняя ссылка на https://activepluse.ru в финале
— ПОЛНОТА: статья должна полностью отвечать на запрос пользователя — не обрывай мысль, не делай "воды"
— ДОСТОВЕРНОСТЬ: только проверенные факты из web_search. Никаких выдуманных исследований или цифр
— ЧИТАЕМОСТЬ: предложения не длиннее 25 слов, абзацы 3-5 предложений, избегай сложных канцеляризмов

ФОРМАТ ОТВЕТА — строго JSON:
{
  "title": "заголовок статьи (50-60 символов, главный ключевой запрос в начале)",
  "description": "SEO описание (150-160 символов, с ключевым запросом, побуждающее к клику)",
